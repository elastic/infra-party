locals {
  name_prefix                  = "${var.resource_prefix}-nlb-passthrough-int"
...
# Health check probes for internal load balancers come from these Google IP ranges
...
# Internal IP address for the load balancer (must be from the subnet's IP range)
...
# Internal forwarding rule - must specify network and subnetwork
